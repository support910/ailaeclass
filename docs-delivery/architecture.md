...
ClassroomIO 是一個現代化的學習管理系統（LMS），採用前後端分離的微服務架構。系統支援多租戶模式，每個組織擁有獨立的子域名和完整的數據隔離。
...
ClassroomIO 採用 **pnpm monorepo** 結構，使用 **Turborepo** 進行構建編排。
...
ClassroomIO 支援多租戶模式，每個組織（Organization）是一個獨立的租戶：
...
**路由機制：**
- 多租戶模式：`{org-slug}.classroomio.com`
- 單組織模式：`app.classroomio.com`（通過 `PUBLIC_SINGLE_ORG_SITE_NAME` 啟用）
...
ClassroomIO 使用 Supabase Auth 進行用戶認證，支援：
