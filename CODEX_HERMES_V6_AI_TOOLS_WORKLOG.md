...
| 2026-06-24 | AI privacy and language | Codex added AI-generated content disclaimers, privacy/data-use notices, and a Traditional Chinese / Simplified Chinese / English language switch across the AI hub and individual tool pages. Tool labels, status badges, forms, result headings, copy output, and endpoint prompts now follow the selected output language. | `pnpm --filter @cio/dashboard run build` passed. Existing build warnings are unchanged and outside the AI module. Secrets remain in local env only and are not documented in this file. |
