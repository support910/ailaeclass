...
### 2026-08-09 · 考试保存 / 列表提速 + 假按钮修复（本地提交，**未推送**）

**触发**：用户反馈考试页「显示加载中、太单调、速度很慢」，以及 CSV 上传慢。

#### 考试列表接口 2385ms → 1240ms（−48%）

`GET /api/org/{id}/exams` 原本串行 6~7 个 Supabase 往返。三处改动：
并行化互不依赖的权限检查与 group 查询；`course → lesson → exercise`
三步合成**一次嵌套查询**（接入前单独实测 1926ms → 313ms，返回 ID 集合完全一致）；
回收站清理移出关键路径（它原本是一次 SELECT 加**每个过期考试一次串行 DELETE**）。

⚠️ 清理不能简单地不 await——原返回**依赖 purge 先删掉过期项**。
现在返回里按同样条件（`deleted_at` 非空且 `delete_after <= now`）自行过滤，
正确性不再依赖清理是否跑完。

#### 考试保存 35.2s → 7.6s（−79%）——这才是 CSV 导入慢的真因

**CSV 导入本身是纯客户端的**（只把题目追加到本地数组，不写库），
慢的是导入后点保存。`POST /api/exercises/{id}` 是**嵌套串行循环**：

```
for (每道题) {              // 20 道，串行
    await upsert question
    for (每个选项) await insert/upsert option   // 每选项一次往返
    await update metadata
}
```

20 题 × 4 选项 = 100+ 次串行往返。实测 **35221ms**。

两步优化：① 同一题内的选项并行写入（35.2s → 19.4s）；
② 题目循环改为**分批并发，每批 5 道**（19.4s → 7.6s）。
特意不用全并发——20 题 × 4 选项会同时开上百个连接。

**验证**：读库逐项核对 14/14（题目数、顺序、标题、分值、选项数、选项文本、
正确答案位置、无重复选项、删除路径），测试数据已清理。
另跑 考试只读 16/16、考试 API 正确性 13/13、导航 22/22、安全验收 14/14。

#### 加载态

考试列表的「加载中」原本是一行居中文字。改为与真实内容形状一致的骨架
（两个模式分区 + 卡片 + 状态标签 + 四个元数据字段），数据到达不跳动。
原文字保留给屏幕阅读器（`sr-only` + `aria-busy`）。

#### 假按钮修复

`Courses/components/List` 的四个溢出菜单项原本只弹 `alert('WIP: ...')`，
而**同样四个功能在 Card 视图里是真的**。改为复用 Card 的实现（同 store、同路由）。
`audience` 页的「导出」原本弹 `alert('This feature is coming soon')`，
改为真实导出 CSV（带 BOM，只含该页已渲染的列）。

⚠️ **但这两个菜单目前都不可达**：`Courses/index` 里
`canManageActions = !isExplore && $globalStore.isStudent === false`，
而 `globalStore.isStudent` 初始为 `undefined`、**只在 `CourseContainer` 内赋值**，
课程列表页从不挂载它。`undefined === false` 为 false，
所以管理菜单对谁都不显示——已实测 admin/teacher × 网格/列表 四种组合菜单数均为 0。
**要不要打开是产品决策**（等于决定谁能从列表直接删课，牵涉教师课程边界规则），故未擅自更改。

#### 我犯的一个错误（记录备查）

改 `audience/+page.svelte` 时写了 TS 类型标注 `value: unknown`，
但该文件 `<script>` **没有 `lang="ts"`**，导致整页编译失败白屏。
当时跑完 svelte-check 只 grep 了关键词、没输出就判断"通过"——**这个判断方法不对，应看退出码**，
是实测页面才发现。已修复。
...
### 2026-08-08 · 控制台 UI 改版 + 切换动效（已随 `cc0cec0` 一并提交）
