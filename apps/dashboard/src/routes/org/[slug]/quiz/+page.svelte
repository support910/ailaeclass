...
<svelte:head>
  <title>{$t('components.quiz.interactive_quizzes')}</title>
</svelte:head>

<section class="w-full max-w-6xl mx-auto">
  <div class="py-10 px-5">
    <div class="flex items-center justify-between mb-10">
      <h1 class="dark:text-white text-2xl md:text-3xl font-bold">{$t('components.quiz.interactive_quizzes')}</h1>
      {#if $isMobile}
        <PrimaryButton onClick={() => ($createQuizModal.open = true)}>
          <Add size={24} />
        </PrimaryButton>
      {:else}
        <PrimaryButton label={$t('components.quiz.create_quiz')} onClick={() => ($createQuizModal.open = true)} />
      {/if}
    </div>
