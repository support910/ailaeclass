<script lang="ts">
  ...
  $: if (!isFetching && hasFetchStarted && !hasFetchCompleted) {
  ...
  function onNextQuery(lessons) {
    ...
    if (!isFetching && hasFetchCompleted && lessons.length === 0) {
      goto(`/courses/${data.courseId}/lessons`);
    }
  }
  ...
  $: {
    if (!isFetching && hasFetchCompleted && shouldGoToNextLesson) {
      onNextQuery(getLessons());
    }
  }
