<script lang="ts">
  ...
  function pageSetup() {
    console.log(
      `Welcome to ${BRAND.name}, we are grateful you chose us.`,
      $page.url.host,
...
{:else if data.skipAuth}
  <PlayQuiz />
{:else if isSingleOrgMode() && !path}
  <!-- Single-org: root "/" lets getProfile handle routing based on role -->
  <slot />
...
<style>
  ...
  :global(:root) {
    --main-primary-color: rgba(14, 115, 114, 1);
    --border-color: #eaecef;
