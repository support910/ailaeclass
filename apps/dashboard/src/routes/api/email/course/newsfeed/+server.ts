...
const sendEmailNotification = async (
  ...
  const postLink = `https://${feed.org.siteName}.5gnumultimedia.com/courses/${feed.courseId}?feedId=${feed.id}`;

  if (comment) {
    const emailData = [
      {
        from: `"${feed.org.name}" <notify@mail.5gnumultimedia.com>`,
      ...
      `,
        replyTo: 'noreply@5gnumultimedia.com'
      }
    ];
