...
<AuthUI {supabase} {handleSubmit} showOnlyContent={true} showLogo={!success}>
  {#if success}
    <div class="mt-4 w-full flex items-center justify-center flex-col">
      <EmailSentIcon />
      <h3 class="dark:text-white text-xl font-semibold my-3">{$t('forgot_password.email_sent')}</h3>
      <p class="dark:text-white text-md mb-6 text-center">
        {$t('forgot_password.email_sent_description', { email: fields.email })}
      </p>
    </div>

    <div class="my-4 w-full flex justify-end items-center flex-col">
      <PrimaryButton
        label={$t('forgot_password.okay')}
        type="button"
  ...
  {:else}
    <div class="mt-4 w-full">
      <h3 class="dark:text-white text-xl font-semibold my-3">{$t('forgot_password.heading')}</h3>
      <p class="dark:text-white text-sm mb-6">{$t('forgot_password.description')}</p>
      <TextField
        label={$t('login.email')}
    ...
    <div class="my-4 w-full flex justify-end items-center flex-col">
      <PrimaryButton
        label={$t('forgot_password.reset_button')}
        type="submit"
        className="sm:w-full w-full mb-4"
        isDisabled={loading}
        isLoading={loading}
      />
      <PrimaryButton
        label={$t('forgot_password.cancel')}
        type="button"
