...
export async function getOrganizations(userId: string, isOrgSite?: boolean, orgSiteName?: string) {
  ...
  if (Array.isArray(memberData) && memberData.length) {
    ...
    // Single-org mode: always pin to the configured org, no localStorage lookup
    if (isSingleOrgMode()) {
      const singleOrgSiteName = getSingleOrgSiteName()!;
      const orgData = orgsArray.find((org) => org.siteName === singleOrgSiteName);
      ...
      if (localStorage) {
        const lastOrgSiteName = localStorage.getItem('ailaeclass_org_sitename');
